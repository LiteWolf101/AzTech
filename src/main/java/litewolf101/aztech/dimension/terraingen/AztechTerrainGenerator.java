package litewolf101.aztech.dimension.terraingen;
 ...
 */
public class AztechTerrainGenerator {
				...
				for(int height32 = 0; height32 < 32; ++height32) {
					...
					for(int h = 0; h < 8; ++h) {
						...
						for(int x = 0; x < 4; ++x) {
							...
							for(int z = 0; z < 4; ++z) {
								if(height < 2) {
									primer.setBlockState(x4 * 4 + x, height32 * 8 + h, z4 * 4 + z, BlocksInit.ANCIENT_BEDROCK.getDefaultState());
								}
								else if((d15 += d16) > 0.0D) {
									primer.setBlockState(x4 * 4 + x, height32 * 8 + h, z4 * 4 + z, Blocks.STONE.getDefaultState());
								}
								else if(height == 180) {
									primer.setBlockState(x4 * 4 + x, 180, z4 * 4 + z, BlocksInit.ANCIENT_BEDROCK.getDefaultState());
								}
							}
