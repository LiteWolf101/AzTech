 ...
 */
public class AzTechPortalItem extends Item implements IHasModel{

    private final Block block;

    public AzTechPortalItem(String name, Block block) {
        setUnlocalizedName(name);
        setRegistryName(name);
        setCreativeTab(AzTech.CREATIVE_TAB);
        this.block = BlocksInit.AZTECH_PORTAL;
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
        ...
        else
        {
            ...
            if (player.canPlayerEdit(pos, facing, itemstack) && BlocksInit.AZTECH_PORTAL.canPlaceBlockAt(worldIn, pos))
            {
                placeBlock(worldIn, pos);
                worldIn.playSound(player, pos, SoundEvents.BLOCK_METAL_BREAK, SoundCategory.BLOCKS, 1.0F, 0.8F);
                itemstack.shrink(1);
    ...
    public static void placeBlock(World worldIn, BlockPos pos) {
        ...
        //    }
        //}
        worldIn.setBlockState(pos, BlocksInit.AZTECH_PORTAL.getDefaultState().withProperty(HALF, EnumHalf.EnumType.BOTTOM));
        worldIn.setBlockState(pos.up(), BlocksInit.AZTECH_PORTAL.getDefaultState().withProperty(HALF, EnumHalf.EnumType.TOP));
    }
