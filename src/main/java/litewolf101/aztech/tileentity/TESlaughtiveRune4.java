 ...
 */
public class TESlaughtiveRune4 extends TileEntity implements ITickable {
		...
		for(EntitySlime slime : nearbySlimes) {
			if(slime.deathTime > 0) {
				if(slime.deathTime == 1) {
					world.setBlockState(pos, BlocksInit.SLAUGHTIVE_RUNE.getDefaultState().withProperty(STAGE, EnumStage.EnumType.STAGE_5));
				}
			}
		}
	}
